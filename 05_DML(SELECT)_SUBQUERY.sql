...
WHERE SALARY >= (SELECT ROUND(AVG(SALARY))
 ...
 /*
    서브쿼리 구분
    서브쿼리를 수행한 결과값이 몇행 몇열 이냐에 따라서 분류가 됨.
    - 단일행 (단일열) 서브쿼리 : 서브쿼리를 수행한 결과값이 오직 1개일 때 (한칸의 칼럼값으로 나올 때
    - 다중행 (단일열) 서브쿼리 : 서브쿼리를 수행한 결과값이 여러 행일 때 
    - (단일행) 다중열 서브쿼리 : 서브쿼리를 수행한 결과값이 여러 열일 때 
    - 다중행 다중열 서브쿼리 : 서브쿼리를 수행한 결과값이 여러행 여러열 일떄
    
    => 서브쿼리를 수행한 결과가 몇행 몇열이나야 떼라 사용가능한 연사자가 달라진다.
 */
 /*
...
WHERE SALARY = (SELECT MIN(SALARY)
                FROM EMPLOYEE);
-- 노옹철 사원의 급여보다 많이 받는 사원들의 사번 이름, 부서코드, 급여 조회.
...
-- 부서별 급여 합이 가장 큰 부서 하나만을 조회, 부서코드, 급여 합
...
HAVING SUM(SALARY) =(SELECT MAX(SUM(SALARY))
 ...
 /*
    ...
    - IN(10,20,30...) 서브쿼리 : 여러개의 결과값중에서 하나라도 일치하는 것이 있따면 / NOT IN(일치하는것이 없을 때)
    - > ANY(10,20,30,...) : 여러개의 결과값중에서 "하나라도" 클 경우 즉, 여러개의 값중에서 가장 작은 값보다 클 경우
    - < ANY(10,20,30,...) : 여러개의 결과값중에서 "하나라도" 작을 경우 즉, 여러개의 값중에서 가장 큰 값보다 클 경우
...
-- 이오리 또는 하동운 사원과 같은 직급인 사원들을 조회하시오
...
WHERE E.JOB_CODE = J.JOB_CODE
    AND JOB_NAME = '과장'; --2500000 ,2200000, 3760000
...
-- 과장 직급임에도 불구하고 "모든" 차장 직급의 급여보다 더 많이 받는 사람
...
/*
    ...
    서브쿼리 조회 결과가 값은 한행이지만, 나열된 칼럼의 갯수가 여러개인 경우...
*/
...
-- 다중열 서브쿼리( 비교할 값의 순서를 맞추는 게 중요하다)
-- (비교대상칼럼1 , 비교대상칼럼2) = (비교할 값1, 비교할값 2) -> 서브쿼리로 제시
SELECT EMP_NAME, DEPT_CODE, JOB_CODE, HIRE_DATE
FROM EMPLOYEE
...
-- 박나라 사원과 같은 직급코드, 같안 사수사번을 가진 사원들의 사번, 이름, 직급코드, 사수사번 조회
-- 다중열 서브쿼리로 작성하기 -> 한번에 안된다 싶으면 나눠서
...
-- 인라인뷰 사용 : 사원명만 골라내기 (보너스 포함 연봉이 3000만원 이상인 사원들의 이름만)
SELECT EMP_NAME
FROM(
    SELECT EMP_ID, EMP_NAME, (SALARY + SALARY * NVL(BONUS, 0)) * 12 보너스포함연봉, DEPT_CODE
    FROM EMPLOYEE
...
WHERE ROWNUM <= 5
ORDER BY SALARY DESC;
-- 에러원인 : ORDER BY 로 정렬하기 전에 이미 ROWNUM의 순번이 매겨져 있기 때문.
...
SELECT ROWNUM , S.* --DEPT_CODE, ROUND(AVG(SALARY)) 평균급여
FROM {
        SELECT DEPT_CODE, ROUND(AVG(SALARY)) AS SALARY
        FROM EMPLOYEE
        GROUP BY DEPT_CODE 
        ORDER BY 2 DESC
      ) S
...
-- 선 정렬 후 순위 매개기를 해야한다. -> 우선적으로 인라인뷰로 ORDER BY절을 만들고 그 후에 메인 쿼리에서 순번을 붙임.
...
/*
    ...
    - DENSE_RANK() OVER(정렬기준) : 공동 1위가 3명이라면 그 다음 순위는 2위로.
