...
SELECT EMP_NAME 이름, SALARY*12 연봉, (SALARY+(SALARY*BONUS))*12 "총수령액(보너스포함)" 
        , (SALARY+(SALARY*BONUS))*12 - (SALARY*12*0.3) 실수령액
FROM EMPLOYEE;   
...
SELECT EMP_NAME 이름, SALARY 월급, (SALARY+(SALARY*BONUS))*12 - (SALARY*12*0.3) 실수령액, HIRE_DATE 고용일
FROM EMPLOYEE
...
--10. EMPLOYEE테이블에 DEPT_CODE가 D9이거나 D5인 사원 중
 ...
 FROM EMPLOYEE
 WHERE DEPT_CODE IN ('D9', 'D6') AND HIRE_DATE < '02/01/01';
...
SELECT *
FROM EMPLOYEE
WHERE EMAIL LIKE  '_4%' AND DEPT_CODE IN('D9', 'D6') AND HIRE_DATE BETWEEN '90/01/01' AND '00/12/01'
      AND SALARY >= 2700000;
--15. EMPLOYEE테이블에서 사원 명과 직원의 주민번호를 이용하여 생년, 생월, 생일 조회
SELECT EMP_NAME 사원명, EMP_NO 주민번호, TO_CHAR(SUBSTR(EMP_NO,1,2)) || '년' 생년
      , TO_CHAR(SUBSTR(EMP_NO,3,2)) || '월' 생월
      , TO_CHAR(SUBSTR(EMP_NO,5,2)) || '일' 생일
FROM EMPLOYEE;
...
SELECT EMP_NAME 사원명, FLOOR (ABS(HIRE_DATE-SYSDATE)) || '일' 근무일수1
, FLOOR(SYSDATE-HIRE_DATE) || '일' 근무일수2
FROM EMPLOYEE;
...
SELECT EMP_NAME 사원명, TO_CHAR(SALARY, 'L9,000,000') "급여 조회"
FROM EMPLOYEE;
...
SELECT EMP_NAME, DEPT_CODE, TO_CHAR(TO_DATE(SUBSTR(EMP_NO,1,6)),  'YY"년" MM"월" DD"일"') 생년월일
       ...
       , TRUNC(MONTHS_BETWEEN(TRUNC(SYSDATE) , TO_DATE(SUBSTR(EMP_NO,1,6), 'RRMMDD'))/12) 나이
FROM EMPLOYEE;
