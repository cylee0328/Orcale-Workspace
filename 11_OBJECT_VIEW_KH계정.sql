/*
 ...
 STORED-VIEW : 영구적으로 저장하고 사용하는 VIEW(서브쿼리를 저장시켜 놓음)
*/
...
-- 뷰 특징
-- 1. 컬럼에 대해 산술연산 및 함수의 실행결과를 포함할 수 있다. 단, 반드시 별칭을 부여해야함.
...
-- DECODE 함수를 이용해서 사원의 이름, 사번, 직업명, 남자인지 여자인지 성별정보, 근속년수 를 VIEW로 저장
...
/*
    INSERT, UPDATE, DELETE 수행할 때 -> 가능하긴한데 실제 테이블에 담겨있는 값이 변경됨.
    만약에 내가 업데이트 하고자하는 칼럼이 가상칼럼이라면 DML이 불가능함.
*/
...
UPDATE V_READONLY SET EMP_NAME = '민경민'; --> 뷰라서 안됨
