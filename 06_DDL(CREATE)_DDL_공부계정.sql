...
/*
    ...
    - CHAR(바이트수) : 최대 2000BYTE까지 지정가능.
                      고정길이(아무리 적은 값이 들어와도 빈공간은 공백으로 채워서 처음 할당한 크기를 유지함)
...
CREATE TABLE MEMBER ( -- 동일한 테이블명 중복 불가.
    MEMBER_ID VARCHAR2(20), -- 대소문자 구분하지않음. 따라서 낙타봉표기법의미없다. => 언더바로 구분.
...
INSERT INTO MEMBER VALUES(null,null,null,null); -- 아이디,비번,이름에 null 값 존재해도 될까요??

-- 위의 null 값이나 중복된 아이디값은 유효하지 않는 값들이다.
...
/*
    ...
    - 제약조건이 부여된 컬럼에 들어온 데이터에 문제가 있는지 없는지 자동으로 검사할 목적.
    ...
    => 즉, NULL 값이 절대 들어와서는 안되는 칼럼에 부여하는 제약조건
      삽입/ 수정시 NULL 값을 허용하지 않도록 하는 제한하는 제약조건.
...
INSERT INTO MEN_NOTNULL VALUES(2 , 'user01', 'pass01', null, null, null, null);
...
-- 테이블레벨 방식 : 모든 칼럼을 다 기술하고 그 이후에 제약조건을 나열
CREATE TABLE MEM_UNIQUE(
    MEM_NO NUMBER NOT NULL,
    MEM_ID VARCHAR2(20) NOT NULL,
    MEM_PWD VARCHAR2(20) NOT NULL,
...
-- CHECK 제약조건 위배.
INSERT INTO MEM_CHECK
VALUES(3, 'user03', 'pass01', '민경민3', NULL, '010-1111-1111', null, SYSDATE);
-- CHECK 제약조건에 NULL값도 추가 가능함.
-- NULL값을 추가적으로 못들어오게 하고 싶다면 ?? NOT NULL 제약조건을 걸어줘야 한다.

/*
    DEFAULT 설정
    트정 컬럼에 들어올 값에 대한 기본 설정.(제약조건은 아님)
    
    예) 회원가입을 컬럼에 회원정보가 삽입된 순간의 시간을 기록하고 싶다.
...
/*
    INSERT INTO MEM_CHECK
    VALUES(값들 나열) ; -- 모든 칼럼의 값을 제시해줘야함.
    
    INSERT INTO MEM_CHECK(C추가할 칼럼명들 나열)
...
/*
    ...
    컬럼에 부여하는 제약조건
    => 다른테이블을 참조한다 라고 표현
        즉, 참조된 다른 테이블이 제공ㅎ고 있는 값만 해당칼럼에 들어갈 수 있다.
        EX) KH계정에서
            EMPLOYEE 테이블의 DEPT_CODE와 DEPARTMENT 테이블의 DEPT_ID값
            -> DEPT_CODE 에는 DEPARTMENT테이블의 DEPT_ID에 존재하는 값만 들어올 수 있다.(NULL값 도 가능)
            
    => FOREIGN KEY 제약조건으로 다른 테이블과 관계를 형성할 수 있다. (JOIN)
    [표현법]
    > 칼럼레벨 방식
    칼럼명 자료형 CONSTRAINT 제약조건명 REFERENCES 참조할 테이블명 (참조할 칼럼명)
    
    > 테이블레벨방식
    CONSTRAINT 제약조건명 FOREIGN KEY(컬럼명) REFERENCES 참조할 테이블명 (참조할 컬럼명)
    
    참조할 테이블 == 부모테이블
    생략가능한 것 : CONSTRAINT 제약조건명, 참조할 컬럼명.
    => 생략 시 자동으로 참조한 테이블의 PRIMARY KEY에 해당되는 칼럼이 참조할 컬럼으로 사용됨.
...
CREATE TABLE MEM_GRADE(
    ...
    GRADE_NAME VARCHAR2(20) NOT NULL -- 등급 명/문자열('일반회원', '우수회원' , '운영자',...) + 제약조건
);
...
CREATE TABLE MEM(
    ...
    MEM_PWD VARCHAR2(20) NOT NULL,
    MEM_NAME VARCHAR2(20) NOT NULL,
    GRADE_ID CHAR(2) REFERENCES MEM_GRADE(GRADE_CODE), --컬럼레벨방식 외래키 지정.
    GENDER CHAR(1) CHECK (GENDER IN ('F', 'M')) NOT NULL,
    PHONE VARCHAR2(15),
    EMAIL VARCHAR2(30),
    MEM_DATE DATE DEFAULT SYSDATE NOT NULL -- DEFAULT 설정을 먼저 하고나서 제약조건을 추가해야함.
...
WHERE GRADE_CODE = 'G1';
-- 자식테이블에서(MEM)에서 GRADE_ID값이 G1인 녀석이 이미 존재하고 있기 때문에 함부로 삭제할 수 없다.

-- 외래키 제약조건 부여시 삭제에 대한 옵션을 추가해주면 됨.
-- => 기본적으로 삭제제한 옵션이 있음.
...
/*
    ...
    FOREIGN KEY 삭제 옵션
    - ON DELETE SET NULL : 부모데이터를 삭제할 떄 해당 데이터값을 사용하는 자식데이터를 NULL로 바꾸겠다.
...
/*
    굳이 외래키 제약조건이 걸려있지 않더라고 JOIN이 가능함.
    다만, 두 컬럼에 동일한 의미의 데이터가 담겨있어야함. (자료형이 같아야되고, 담긴값의 종류, 의미도 비슷해야함)  
*/
...
-- 전체사원들 중 급여가 300만원 이상인 사원들의 사번, 이름, 부서코드, 급여칼럼 복제하시오(내용물 포함)
