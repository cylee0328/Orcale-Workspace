/*
    ...
    데이터를 제어 언어, DB에대한 보아느 , 무결성, 복구 등 DBMS를 제어하기위한 언어
    권한 부여(GRANT, REVOKE) ,
...
-- 3. SAMPLE 계정에게 테이블 스페이스할당.
ALTER USER SAMPLE2 QUOTA 2M ON SYSTEM;
ALTER USER SAMPLE2 QUOTA 2M ON USERS;

-- 4. SAMPLE 계정에게 VIEW를 만들 수 있는 권한부여
GRANT CREATE VIEW TO SAMPLE2;

-- 5. SAMPLE 계정에서 KH계정의 테입르 중 EMPLOYEE 테이블을 조회할 수 있는 권한 추가
