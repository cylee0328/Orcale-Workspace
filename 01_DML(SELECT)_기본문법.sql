...
/*
    <컬럼값을 통한 산술연산> 
    조회하고자하는 칼럼들을 나열하는 SELECT절에서 산술연선(+-/*)를 기술해서 결과를 조회할 수 있다.
*/
...
-- DATE타입끼리도 연산가능( DATE =>년,월,일,시,분,초)
-- 오늘날짜 : SYSDATE
...
/*
    ...
    AS 키워드를 붙이든 안 붙이든간에
    별칭에 특수 문자나 띄어쓰기가 포함될 경우 ""를 묶어서 표기해줘야함.
*/
...
/*
    <DISTINCT>
    조회하고자 하는 칼럼에 중복된 갑을 딱 한번만 조회하고자 할 때 사용
    해당 칼럼명 앞에 기술
...
-- EMPLOYEE테이블에서 급여가 400만원 이상의 사원들만 조회
SELECT *
FROM EMPLOYEE 
...
-- 1. EMPLOYEE 테이블에서 급여가 100만원 이상인 사원들의 이름, 급여, 입사일 조회
...
-- EMPLOYEE 테이블에서 부서코드가 D9이면서, 급여가 500만원 이상인 사원들의 이름, 부서코드, 급여조회
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
...
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE ENT_YN = 'N' AND (DEPT_CODE = 'D6'  OR SALARY >= 3000000);

-- 급여가 350만원 이상이고 600만원 이하인 사원들의 이름, 사번, 급여, 직급코드 조회
SELECT EMP_NAME, EMP_NO, SALARY, JOB_CODE
FROM EMPLOYEE
...
/*
    ...
    '%" : 0글자 이상
          ...
          비교대상 칼럼명 LIKE '%문자' => 컬럼값 중에 '문자'로 끝나는 것을 조회
          비교대상 칼럼명 LKIE '%문자%' => 컬럼값 중에 '문자'가 포함되는 것을 조회
          
    '_' : 1글자
          비교대상 칼럼명 LIKE '_문자' : 컬럼값 중에 '문자' 앞에 무조건 1글자가 존재하는 경우 조회
           비교대상 칼럼명 LIKE '__ 문자' : 컬럼값 중에 '문자' 앞에 무조건 2글자가 존재하는 경우 조회
*/
...
-- 부서배치는 받지 않았지만 보너스는 받는 사원의 모든컬럼조회
SELECT *
FROM EMPLOYEE
...
-- 직급코드가 J1 J3 J4 가 아닌 사원들의 모든 칼럼 조회
SELECT *
FROM EMPLOYEE
...
/*
    ...
    최종 조회된 결과문들에 대해서 정렬 기준을 세워주는 구문.
  
    [표현법]
    SELECT 컬럼명1, 컬럼명2, ...
    FROM 테이블명
    WHERE 조건식(생략가능)
    ORDER BY [정렬기준으로 세우고자 하는 칼럼명,별칭,컬럼순번] [ASC(오름차순)(자동)/DESC(내림차순)](생략가능) [NULLS FIRST/NULLS LAST(자동)] (생략가능)
*/
...
--> ORDER BY 절은 숫자뿐만아니라 문자열, 날자에 대해서도 정렬가능하다.
